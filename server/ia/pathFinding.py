        ...
        while len(to_visit) > 0 and nb < 1000:
            ...
            for pix in to_visit:
                ...
                if self.distance(pix) < self.distance(current_pix) or (
                    self.distance(pix) == self.distance(current_pix)
                    and pix.dist_e < current_pix.dist_e
                ):
                    current_pix = pix
            ...
            for pos, dist in self.adj(current_pix.get_tuple()):
                ...
                if (
                    ...
                    or nb_frame < pix.len_path
                ):
                    # test si il est arrive ou si il a calcule asse d'image
                    return pix.get_path()

                elif (
                    pix.get_tuple() not in visited
                    and not pix.in_list(to_visit)
                    and not self.hitbox.get_server_collided()
                ):
